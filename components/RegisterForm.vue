<template>
    ...
    <form @submit.prevent="submit" :class="[app.mobile ? 'max-w-[95vw]' : '']"
        ...
        <div class="mb-4">
            ...
            <input v-on:focus="onEmail = true" v-on:blur="onEmail = false" v-model="email" id="email" type="email"
                class="w-full px-4 py-2 rounded-md bg-[var(--article-bg)] border border-[var(--border-color)] focus:outline-none focus:ring"
                required />
        </div>

        <div v-if="onEmail" class="mb-4 text-sm text-[var(--secondary-text-color)]">Input an active email address</div>
        ...
        <div v-if="onOTP" class="mb-4 text-sm text-[var(--secondary-text-color)]">
            {{ otpMsg }}
            <br>
            <template v-if="otpStatus === 'sent'">You can resend your verification code in: {{ otpCountdown
                }}</template>
        </div>
        ...
        <div class="mb-4 relative">
            ...
            <input v-on:focus="onPassword = true" v-on:blur="onPassword = false" v-model="password" id="password"
...
// state where window focused on, to inform someth
...
// state to show verification code message and request countdown
const otpStatus = ref('')
...
// hide password or show?
const passwordShown = ref(false)
const confirmPasswordShown = ref(false)

// register status to be shown
const status = ref('')
const isError = ref(false)
const isLoading = ref(false)

// verification code valid for 2 minute before user can request new one, this is the countdown logic, shown under the verification field
...
async function submit() {
    ...
    const fetch = await register(email.value, password.value, confirmPassword.value, otp.value)
