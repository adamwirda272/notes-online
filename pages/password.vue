<template>
    <form @submit.prevent="submit" :class="[context.isMobile ? 'max-w-[95vw]' : '']" class="h-auto w-[450px] rounded-md border border-[var(--shadow-color)] shadow-lg p-8 text-[var(--text-color)] bg-[var(--header-bg)]">
        ...
        <div class="mb-4 relative">
            ...
            <input v-on:focus="onPassword = true" v-on:blur="onPassword = false" v-model="newPassword" id="newPassword" :type="passwordShown ? 'text' : 'password'" minlength="6" maxlength="25" class="w-full px-4 py-2 rounded-md bg-[var(--article-bg)] border border-[var(--border-color)] focus:outline-none focus:ring" required/>
        ...
        <button type="submit" class="w-full mt-14 py-2 border rounded-md flex items-center justify-center cursor-pointer hover:border-[var(--primary-color)] hover:text-[var(--primary-color)]">
            Submit
        </button>
